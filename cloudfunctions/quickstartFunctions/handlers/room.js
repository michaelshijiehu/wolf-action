module.exports = (ctx) => ({
    ...
    while (attempts < 10) {
      ...
      if (existing.total === 0) {
        const init = {
          ...
          game_state: { status: 'waiting', day_count: 0, phase: 'setup', sub_phase: 'ready', sheriff_seat: null, sheriff_candidate_seats: [], lovers: [], last_night_deaths: [], last_revealed_deaths: [], last_dream_catcher_target: null, deaths_announced: true, voting_history: [], phase_version: 0, transition_lock: null },
  ...
  startGame: async (ev) => {
    ...
    await ctx.db.collection('game_rooms').doc(room._id).update({
      data: {
        players,
        config: { player_count: players.length, roles },
        'game_state': initialGameState,
        current_round_actions: ctx.getInitialActions(),
        timeline: [{ day: 1, phase: 'day', text: '游戏开始', timestamp: new Date() }],
        updated_at: new Date()
      }
    });
    return { success: true };
