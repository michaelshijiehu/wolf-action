...
module.exports = Behavior({
  ...
  methods: {
    ...
    getPhaseDuration(gameState) {
      ...
      if (['sheriff_voting', 'day_voting', 'pk_voting', 'werewolf_phase', 'voting'].includes(subPhase)) return 30;
      ...
      if (['cupid_phase', 'lover_phase', 'guard_phase', 'seer_phase', 'witch_phase', 'hunter_phase', 'hunter_action', 'wild_child_phase', 'magician_phase', 'dream_catcher_phase', 'wolf_beauty_phase', 'gargoyle_phase', 'merchant_phase', 'silencer_phase', 'gravekeeper_phase'].includes(subPhase)) return 30;
      return 3;
    },
    ...
    triggerCountdown(gameState) {
      ...
      // 手动模式：执行正计时逻辑
      if (gs.game_state.is_manual_mode) {
        this.startManualCountUp(gs);
        return;
      }
    ...
    startManualCountUp(gs) {
      this.stopCountdownTimer();
      const startTime = new Date(gs.game_state.last_transition_at || gs.game_state.start_time).getTime();
    ...
    startCountdown(seconds) {
      ...
      this.countdownTimer = setInterval(() => {
        // 如果中间切换到了手动模式，停止倒计时并重新触发
        if (this.data.gameState?.game_state.is_manual_mode) {
          this.triggerCountdown();
          return;
        }

        const currentVal = parseInt(this.data.countdown);
