...
## Automation & CLI
WeChat DevTools HTTP Service is enabled on port **43868**. You can use `curl` to automate tasks.

*   **Preview**:
    ```bash
    curl "http://127.0.0.1:43868/preview?projectpath=$(pwd)"
    ```
*   **Build NPM**:
    ```bash
    curl "http://127.0.0.1:43868/buildnpm?projectpath=$(pwd)"
    ```
*   **Cloud Function Deploy**:
    ```bash
    # Deploy 'quickstartFunctions'
    curl "http://127.0.0.1:43868/upload/cloudfunction?projectpath=$(pwd)&env=YOUR_ENV_ID&functionname=quickstartFunctions"
    ```
